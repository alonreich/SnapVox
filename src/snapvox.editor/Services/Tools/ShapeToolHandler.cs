...
{
    public abstract class ShapeToolHandler : BaseToolHandler
    {
        ...
        public override void OnPointerMoved(ToolHandlerContext context, ToolPointerEvent e)
        {
            ...
            if (context.IsDrawing)
            {
                ...
                var previewEnd = snappedPoint ?? pos;
        ...
        public override void OnPointerReleased(ToolHandlerContext context, ToolPointerEvent e)
        {
            ...
            if (!altPressed)
            {
                var t = context.SnapToNearbyTarget(endPoint, endPoint);
                if (t != endPoint && ToolHandlerContext.Distance(t, endPoint) <= 8.0)
                {
                    endPoint = t;
                }
            }
